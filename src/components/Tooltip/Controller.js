...
export class Controller extends React.PureComponent {
  ...
  getPosition = (tooltipDimensions = this.state.tooltipDimensions) => {
...
const calculatePosition = (pos, trigger, tooltip) => {
  ...
  const overflowsRight =
    left + tooltip.get("width") + TOOLTIP_BOUNDING_PADDING > windowWidth
  ...
  switch (pos) {
    case "top-right": {
      if (overflowsRight && !overflowsTop)
        return calculatePosition("top-left", trigger, tooltip)
      if (!overflowsRight && overflowsTop)
        return calculatePosition("bottom-right", trigger, tooltip)
      if (overflowsRight && overflowsTop)
        return calculatePosition("bottom-left", trigger, tooltip)
      break
    }
