...
const getRankFromTooltip = (spellId, tooltip) => {
...
const getUnknownSpellIds = () => {
  const allSpellIds = Object.values(talentsById).reduce((reduction, talent) => {
...
export const fetchSpellData = async () => {
  ...
  console.log(
    `Fetching spell data for ${unknownSpellIds.length} unknown spells`
  )

  const promises = unknownSpellIds.slice(0, 20).map(async spellId => {
    if (!spells[spellId.toString()]) {
      console.log(`Loading data for unknown spell: ${spellId}`)

      const res = await fetch(
        `https://classic.wowhead.com/tooltip/spell/${spellId}`
      )
      if (res.status !== 200) {
  ...
  if (getUnknownSpellIds().length > 0) {
    console.log(
      "--- We have more unknown spells: ",
      getUnknownSpellIds().length
    )
    fetchSpellData()
  }
}
